(function () {
	...
	function executeHandleCheckWithOutcome(selectedOutcome) {
		...
		if (handled && !isCatchable) {
			...
			if (!throwButton) {
				throwButton = document.createElement('button');
				throwButton.id = 'throwButton';
				throwButton.textContent = 'Throw to Base';
				throwButton.onclick = performThrow;
				document.getElementById('actionButtonsContainer').appendChild(throwButton);
			}
			throwButton.disabled = false;
		}
	...
	function saveGameState(gameName) {
		const gameState = {
			...
			teamsData: teamsData,
			pitchingTeamIndex: pitchingTeamIndex,
			battingTeamIndex: battingTeamIndex,
			currentPitcherIndex: currentPitcherIndex,
			lastPitcherIndex: lastPitcherIndex,
			...
			useManualRolls: useManualRolls,
			selectOutcomesMode: selectOutcomesMode,
			playLog: playLog
		};
		
		let savedGames = JSON.parse(localStorage.getItem('zblSavedGames')) || [];
		savedGames.push(gameState);
		localStorage.setItem('zblSavedGames', JSON.stringify(savedGames));
	...
	function loadGameState(index) {
		...
		pitchingTeamIndex = gameState.pitchingTeamIndex;
		battingTeamIndex = gameState.battingTeamIndex;
		currentPitcherIndex = gameState.currentPitcherIndex;
		lastPitcherIndex = gameState.lastPitcherIndex;
		currentBatterIndex = gameState.currentBatterIndex;
		pdModifier = gameState.pdModifier;
		brOutcome = gameState.brOutcome;
		outs = gameState.outs;
		currentInning = gameState.currentInning;
		isTopInning = gameState.isTopInning;
		runners = gameState.runners;
		teamScores = gameState.teamScores;
		useManualRolls = gameState.useManualRolls;
		selectOutcomesMode = gameState.selectOutcomesMode;
		playLog = gameState.playLog;
		
		// Update UI
		document.getElementById('pitchingTeamSelect').value = pitchingTeamIndex;
		document.getElementById('battingTeamSelect').value = battingTeamIndex;
		document.getElementById('pitcherSelect').value = currentPitcherIndex;
		document.getElementById('batterSelect').value = currentBatterIndex;
		document.getElementById('manualRollSwitch').checked = useManualRolls;
		document.getElementById('selectOutcomesSwitch').checked = selectOutcomesMode;
		...
		document.getElementById('zone-display').style.display = 'flex';
	}
	...
	function refreshSavedGamesList() {
		...
		if (savedGames.length === 0) {
			container.innerHTML = '<p style="color: #999;">No saved games yet</p>';
			return;
		}
		...
		savedGames.forEach((game, index) => {
			...
			div.innerHTML = `
				<div>
					...
					<br>
					<small style="color: #666;">${game.timestamp} - Inning ${game.currentInning}, ${game.isTopInning ? 'Top' : 'Bottom'}</small>
				</div>
				<div>
					<button class="load-save-btn" data-index="${index}">Load</button>
					<button class="saved-game-delete delete-save-btn" data-index="${index}">Delete</button>
				</div>
			`;
			container.appendChild(div);
			
			// Add event listeners to the newly created buttons
			const loadBtn = div.querySelector('.load-save-btn');
			const deleteBtn = div.querySelector('.delete-save-btn');
	...
	function startNewGame() {
		...
		pitchingTeamIndex = '';
		battingTeamIndex = '';
		currentPitcherIndex = -1;
		lastPitcherIndex = { '0': -1, '1': -1, '2': -1, '3': -1 };
		...
		isTopInning = true;
		runners = { '1b': null, '2b': null, '3b': null };
		teamScores = { pitching: 0, batting: 0 };
		playLog = [];
		tabs = {};
		activeTab = null;
	...
	document.addEventListener('DOMContentLoaded', function () {
		...
		document.getElementById('loadGameBtn').addEventListener('click', function() {
			document.getElementById('saved-games-list').style.display = document.getElementById('saved-games-list').style.display === 'none' ? 'block' : 'none';
			refreshSavedGamesList();
		});
		
		// Set up game buttons
		document.getElementById('saveGameBtn').addEventListener('click', function() {
			const gameName = prompt('Enter a name for this save:');
			if (gameName) {
